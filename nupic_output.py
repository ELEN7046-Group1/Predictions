...
class NuPICOutput(object):
  ...
  @abstractmethod
  def write(self, actualValues, predictedValues, timestamps,
            predictionStep=1):
    pass
...
class NuPICFileOutput(NuPICOutput):
  ...
  def __init__(self, *args, **kwargs):
    ...
    self.outputWriters = []
    self.lineCounts = []
    headerRow = ['DayTotal', 'Prediction', 'TimeStamp']
  ...
  def write(self, actualValues, predictedValues, timestamps,
    ...
    assert  len(actualValues) == len(predictedValues) == len(timestamps)
    ...
    for index in range(len(self.names)):
      ...
      if timestamp is not None:
        outputRow = [actual, prediction, timestamp]
